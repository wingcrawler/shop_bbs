...
public class ImageService extends AdapterService implements BaseService {
	...
	/*
	 * public void saveShopImg(Shop shop, String fileName) { Image image =
	 * this.getByShopId(shop.getId()); image.setShopId(shop.getId()); String
	 * uploadPath = PropertiesUtil.get("upload_path_save"); uploadPath +=
	 * DateUtil.dateToString(new Date(), DateUtil.DATE_FORMATE_1)+"/";
	 * image.setImagePath(uploadPath+fileName); this.save(image); }
	 */
	...
	public void saveProductIndexImg(Product product, String fileName, int indexShow) {
		...
		uploadPath += DateUtil.dateToString(new Date(), DateUtil.DATE_FORMATE_1) + "/";
		Map<String, Object> parm = new HashMap<String, Object>();
		parm.put("productId", product.getId());
		parm.put("indexShow", indexShow);
		List<Image> images = imageMapper.getBeanListByParm(parm);
