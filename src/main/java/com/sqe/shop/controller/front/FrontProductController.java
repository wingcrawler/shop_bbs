...
public class FrontProductController extends BaseFrontController {
	...
	public ModelAndView list(ModelAndView model, Long parentType, Long childType, String typeName, String searchText,
		...
		// 没有一级分类 选取默认分类
		/*
		 * if(parentType==null && childType==null){ productType = new
		 * ProductType(); productType.setTypeLevel(1); PageUtil<ProductType>
		 * typePage = productTypeService.getBeanListByParm(productType, 1, -1);
		 * if(typePage!=null && !typePage.getList().isEmpty()){ productType =
		 * typePage.getList().get(0);
		 * product.setProductTypeId(productType.getId()); }
		 * model.setViewName("shop/404"); return model; }
		 */
